...
def load_channel_data(
    participant: int, channel: str
) -> Tuple[Optional[np.ndarray], Optional[pd.DatetimeIndex]]:
    try:
        temp_df: pd.DataFrame = signal_processing(participant)
        if channel not in temp_df.columns:
            logger.error(f"Channel '{channel}' not found in {participant}")
            return None, None
        if not isinstance(temp_df.index, pd.DatetimeIndex):
            logger.error(f"Input file {participant} does not have a DatetimeIndex.")
            try:
        ...
        logger.error(f"Input file not found: {participant}")
        return None, None
    except Exception as e:
        logger.error(
            f"Error loading data for channel '{channel}' from {participant}: {str(e)}"
        )
        return None, None
...
def calculate_eog_complexity_windowed(
    ...
    for channel in required_cols:
        ...
        data, index = load_channel_data(participant, channel)
